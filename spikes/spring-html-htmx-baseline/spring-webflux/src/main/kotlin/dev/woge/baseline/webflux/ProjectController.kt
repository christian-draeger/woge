...
class ProjectController(
    ...
    suspend fun summary(@PathVariable project: String, model: Model): String {
    ...
    suspend fun tasks(@PathVariable project: String, model: Model): String {
    ...
    suspend fun activity(@PathVariable project: String, model: Model): String {
    ...
    suspend fun createTask(
        @PathVariable project: String,
        @ModelAttribute form: CreateTaskForm,
        @RequestHeader(name = "HX-Request", defaultValue = "false") enhanced: Boolean,
        response: ServerHttpResponse,
        model: Model,
    ): String = when (val result = store.createTask(project, form.title, form.owner, form.dueDate)) {
        null -> throw notFound(project)
        CreateTaskResult.BlankTitle -> {
            response.statusCode = HttpStatus.valueOf(422)
            if (enhanced) {
                populateForm(
                    model,
                ...
                populatePage(
                    ...
                    ownerValue = form.owner.orEmpty(),
                    dueDateValue = form.dueDate?.toString().orEmpty(),
    ...
    suspend fun setTaskStatus(
        ...
        @ModelAttribute form: TaskStatusForm,
        @RequestHeader(name = "HX-Request", defaultValue = "false") enhanced: Boolean,
        model: Model,
    ): String {
        val snapshot = store.setCompleted(project, task, form.completed) ?: throw notFound(project)
        return if (enhanced) {
